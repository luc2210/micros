...
void TIM2_IRQHandler(void){
	...
	if ( TIM2->SR & TIM_SR_CC3IF){ // COMPRUBEA SI HUBO UNA CAPTURA EN EL CANAL 3
			if ( encendido2==0){
...
int main(void)
{
  ...
  GPIOC->MODER &= ~(2<<(9*2)); // PIN C9 MODO INPUT 11 echo TRASERO
  GPIOB->MODER &= ~(3 << (8 * 2));  // Limpia bits NO SE SI ESTO HAY QUE UTILIZARLO SUPUESTAMENTE LIMPIA LOS BITS
  GPIOB->MODER |= (1 << (8*2)); //pin A3 COMO SALIDA PARA EL ZUMBADOR
  ...
  NVIC->ISER[0]= (1<<23); // HABILITAR TIM2 EN NVIC IRQ28
